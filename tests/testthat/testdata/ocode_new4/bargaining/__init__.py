...
class Subsession(BaseSubsession):
    # testvariable = 1  # Not working
    testvariable2 = models.CurrencyField(
        doc="""
        This is a normal Subsession variable.
        """,)

    if (True):
        conditional_testvariable = models.CurrencyField()
...
class Player(BasePlayer):
    request = models.CurrencyField(
        ...
        """,
        min=0,
        label="(Please enter an amount from 0 to 100)",  # Changed for testing!
    )

    request2 = models.CurrencyField(
        doc="""
        Amount requested by this player.
        """,
        min=0,
